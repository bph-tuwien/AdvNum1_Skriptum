...
def fc_coupled_HAM(vec_Tpvp, vec_Tpv, K, Fo, Fow, dt, rho, Cp, Cm, Lv):
    ...
    T, w = vec_Tpv[0:n], vec_Tpv[n:]
    Tp, wp = vec_Tpvp[0:n], vec_Tpvp[n:]
    # we need phi in order to actualise w
    phi = fc_phi_w(w)
    phip = fc_phi_w(wp)
    pv = phi / 100 * fc_pvsat(T)
    pvp = phip / 100 * fc_pvsat(Tp)
    ...
    exp_w = 0.5 * (Fow * np.dot(K, pv))
    imp_w = 0.5 * (Fow * np.dot(K, pvp))
...
while t <= sim_time:
    ...
    result_array = fsolve(fc_coupled_HAM,
                          np.hstack([T, w]),
                          args=(np.hstack([T, w]), K, Fo, Fow, dt, rho, Cp, Cm, Lv))

    # split the result into T and pv
    T_plus, w_plus = result_array[0:n], result_array[n:]
